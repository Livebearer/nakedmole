...
def confirm_rename_gui(old_name, suggested_name):
    ...
    frame = tk.Frame(root)
    tk.Button(frame, text="OK", command=on_accept).pack(side='left', padx=5)
    tk.Button(frame, text="Próximo", command=on_skip).pack(side='left', padx=5)
    frame.pack(pady=10)
